...
public class Telekineticpower : MonoBehaviour
{
    public LayerMask _grabLayer;
    public Camera  _camera;

    //The rigidbody we are currently holding
    private Rigidbody _grabbedRigidbody;

    //The transfor of the rigidbody we are holding
    private Transform  _grabbedTransform; 

    //The offset vector from the object's position to hit point, in local space
    private Vector3 _hitOffsetLocal;

    // The distance we are holding the object at
    private float _currentGrabDistance;

    //The interpolation state when first grabbed
    private RigidbodyInterpolation _initialInterpolationSetting;

    //The difference between player and object rotation, updated when picked up or when rotated by the player
    private Quaternion _rotationDifference;

    //Tracks player input to rotate current object. Used and reset every fixedupdate call
    private Vector3 _rotationInput = Vector3.zero;

    //Transform of the player, that rotations should be relative to
    public Transform PlayerTransform;
    public float _rotationSenstivity = 1.5f;
    ...
    //Input values above this will be considered and intentional change in rotation
    ...
    public float _scrollWheelSensitivity = 5f;
    [Tooltip("The min distance the object can be from the player")]
    public float _minObjectDistance = 2.5f;
    /// <summary>The maximum distance at which a new object can be picked up</summary>
    public float _maxGrabDistance = 50f;
